...
inc_login() {
  echo -e "\n${YEL}Logging into the cluster via backplane...${NC}\n"
...
cco_status() {
  ...
  inc_title "Checking the status for Cloud Credential Operator..."
  ...
  error_conditions=$(echo "$cco_conditions" | jq '.status.conditions | .[] | select(.type == "Degraded" or .type == "Progressing" or .type == "Upgradeable") | select(.message != "All is well")')
  error_messages=$(echo "$error_conditions" | jq '.message')
...
cco_pods() {
  inc_separator
  inc_title "Checking the pods status for Cloud Credential Operator..."
...
cco_pod_logs() {
  ...
  inc_title "Gathering the pod logs for Cloud Credential Operator..."
