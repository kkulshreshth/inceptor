...
inc_get_creds () { 
  ...
  while true; do
    ...
    if (( ${#password_tokens} < 6 )); then login_errors+=("token"); fi
    if (( ((${#password_lowercase} + ${#password_uppercase})) < 3 )); then login_errors+=(" three letters"); fi
    ...
    if (( ${#password} < 14 )); then login_errors+=("14 characters long"); fi
...
main() {
  if [[ ! "${operator//-/}" == "cloudcredential" ]] || [[ -z $cluster_id ]]; then echo -e "${YEL}Wrong operator name or missing cluster ID${NC}"; exit 1; fi
  os_default_browser
  inc_get_creds
  inc_login
  get_basic_info
