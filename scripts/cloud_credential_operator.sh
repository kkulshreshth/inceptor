...
inc_get_creds () { 
  ...
  while true; do
    ...
    pass_tokens=$(grep -oE '[[:digit:]]+' <<< $password | sort -rn | head -n1)
    pass_spechars=$(grep -o '[[:punct:]]' <<< $password | tr -d '\n')
    pass_lowers=$(grep -o '[[:lower:]]' <<< $password | tr -d '\n')
    pass_uppers=$(grep -o '[[:upper:]]' <<< $password | tr -d '\n')

    catched_errors=()
    if (( ${#pass_spechars} < 2 )); then catched_errors+=("special characters"); fi
    if (( ${#pass_tokens} < 6 )); then catched_errors+=("6-digit tokens"); fi
    if (( ((${#pass_lowers} + ${#pass_uppers})) < 3 )); then catched_errors+=("3 letters"); fi
    if (( ${#pass_lowers} < 1 )); then catched_errors+=("one lowercase character (a-z)"); fi
    if (( ${#pass_uppers} < 1 )); then catched_errors+=("one uppercase character (A-Z)"); fi
    if (( ${#password} < 14 )); then catched_errors+=("14 total length"); fi

    if [[ -n "${catched_errors[@]}" ]]; then
      echo -e "${YEL}Password must have at least:${NC}"
      for err in "${catched_errors[@]}"; do
        echo -e "${RED} > $err${NC}"
      done
      echo ""
      continue
