...
get_prometheus_graph_links() {
    ...
    # Store the existing container name in consoleContainer
    consoleContainer=$(podman ps -aq --filter "name=^console-" --format "{{.Names}}")
    echo -e $consoleContainer
    # Check if "consoleContainer" is not empty
    if [ -n "$consoleContainer" ]; then
        # If "consoleContainer" is not empty, run the command to remove the containers
        echo -e "removing console container"
        podman rm -f --all >/dev/null 2>&1
    fi
    ...
    # Open a new terminal, run the command, and store its output
